...
fn test_key_caching() {
    ...
    assert_eq!(
        *count_ref.lock().unwrap(),
        1,
        "Should have called get_key once for lock"
    );
    ...
    let env = manager
        .get_environment(&project.id, "staging")
        .expect("failed to get env");
    manager
        .set_secret(&env.id, "K", "V")
    ...
    assert_eq!(
        *count_ref.lock().unwrap(),
        1,
        "Should not have called get_key again due to caching"
    );
